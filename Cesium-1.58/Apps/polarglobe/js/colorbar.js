function Colorbar() {
	...
	selection.selectAll(".pointer").remove()
	    ...
	    offsetGroup = new_colorbars
            ...
            var eleTitle = offsetGroup.append("svg:g")
                .attr("transform", "translate(0, -10)");
            eleTitle.append("svg:text")
		...
		.attr("class","axis color");
            ...
            fillLegendScale.domain([d3.min(fillLegendScale.domain()), d3.max(fillLegendScale.domain())]);
	    ...
	    selection
            ...
            colorAxisFunction = d3.svg.axis()
                ...
                .orient(axis_orient)
                .tickFormat(function(d) {return d + _suffix;});
...
//            return this;
    ...
    function prettyName(number) {
        ...
        if (comparisontype()!='comparison') {
            ...
            return(Math.round(number*10)/10+suffix+_suffix)
        }
        if (comparisontype()=='comparison') {
    ...
    chart.suffix = function(value) {
        if (!arguments.length) return _suffix;
        _suffix = value;
        return chart;
    }
